...
def scan(view_map,source,asteroids):
    tmp_map = copy.deepcopy(view_map)
    alist = []
    asteroid = source
    ...
    for a in as_tmp:
        if tmp_map[a[1]][a[0]] == '#':
            ...
            next_x = a[0] + rel_x
            next_y = a[1] + rel_y
            while next_x < len(tmp_map[0]) and next_x >= 0 and next_y < len(tmp_map) and next_y >= 0:
                tmp_map[next_y][next_x] = '.'
                next_x = next_x + rel_x
                next_y = next_y + rel_y
...
def main(in_string):
    ...
    while destroyed < 200:
        if len(los) == 0:
            los = scan(view_map,source,asteroids)
            # order them clockwise
            los.sort(key=angle_from_12)
        for a in los:
            view_map[a[1]][a[0]] = '.'
            destroyed += 1
            if destroyed == 200:
...
def angle_from_12(sink):
    global source
    deltaX = sink[0] - source[0]
    deltaY = sink[1] - source[1]
