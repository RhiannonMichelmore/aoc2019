...
def main(in_string):
    ...
    # contains exactly 2 matching digits somewhere (ie arent part of a larger group)
    exprs = []
    for i in range(length-1):
        if i == 0:
            exprs.append(And(targetVec[i] == targetVec[i+1], targetVec[i+1] != targetVec[i+2]))
        elif i == length - 2:
            exprs.append(And(targetVec[i] == targetVec[i+1], targetVec[i] != targetVec[i-1]))
        else:
            exprs.append(And(targetVec[i] == targetVec[i+1], targetVec[i+1] != targetVec[i+2], targetVec[i] != targetVec[i-1]))

    twoExp = Or(exprs)

    finalExp = And(singleExp,forceExp,adjExp,decExp,twoExp)
